 ...
 */
//================================================================================================//
Grey_P1_Matrix::Grey_P1_Matrix(const Control_Data &control_data)
 ...
 */
//================================================================================================//
double Grey_P1_Matrix::mass_average(const std::vector<double> &mass,
 ...
 */
//================================================================================================//
void Grey_P1_Matrix::initialize_solver_data(const Orthogonal_Mesh &mesh, const Mat_Data &mat_data,
                                            const double dt) {
  ...
  for (size_t cell = 0; cell < ncells; cell++) {
    ...
    for (size_t face = 0; face < nfaces; face++) {
      solver_data.face_flux0[cell][face] = mat_data.face_flux[cell][face];
 ...
 */
//================================================================================================//
void Grey_P1_Matrix::build_matrix(const Orthogonal_Mesh &mesh, const double dt) {
  const auto ncells = mesh.number_of_local_cells();
  ...
  for (size_t cell = 0; cell < ncells; cell++) {
    ...
    for (size_t face = 0; face < nfaces; face++) {
      ...
      if (ftype == FACE_TYPE::GHOST_FACE) {
        ...
        face_D[cell][face] = (constants::c * dt) / (3.0 * (1.0 + constants::c * dt * sigma_tr));
        const auto rank = gcomm->put_map[cell][face].first;
        const auto put_index = gcomm->put_map[cell][face].second;
        // populate the put buffer
    ...
    for (size_t face = 0; face < nfaces; face++) {
      ...
      if (ftype == FACE_TYPE::GHOST_FACE) {
        ...
        const double flux_source = -fring * 3.0 * (half_width + next_half_width) /
                                   (constants::c * dt) * solver_data.face_flux0[cell][face] /
                                   constants::c;
        ...
        if (!reflect_bnd[face]) {
          ...
          const double flux_source = fring * 3.0 * half_width / (constants::c * dt) *
                                     solver_data.face_flux0[cell][face] / constants::c;
          solver_data.source[cell] += fring * E0;
          solver_data.source[cell] += flux_source;
          solver_data.diagonal[cell] += fring;
          solver_data.off_diagonal[cell][face] = fring;
          solver_data.flux_source[cell][face] = fring * E0 + flux_source;
        }
      }
    }
  }
 ...
 */
//================================================================================================//
void Grey_P1_Matrix::gs_solver(const double eps, const size_t max_iter, const bool print) {
  Require(max_iter > 0);
  ...
  while (max_error > eps && global_count < max_iter) {
    ...
    while (max_error > eps && inner_count < max_iter) {
      ...
      for (size_t i = 0; i < solver_data.diagonal.size(); i++) {
        ...
        calc_correction(cell_epsilon[i], cell_correction_source[i], solver_data.cell_temperature[i],
                        solver_data.cell_eden[i], sigma_a[i], fleck[i], solver_data.cell_cve[i],
 ...
 */
//================================================================================================//
void Grey_P1_Matrix::calculate_output_data(const Orthogonal_Mesh &mesh, const Mat_Data &mat_data,
...
void Grey_P1_Matrix::calc_correction(double &epsilon, double &correction_source, double &Tstar,
                                     double eden, const double sigma_a, const double fleck,
                                     const double cv, const double volume, const double dt,
                                     const double T0, const double ext_exp_source) {
  double error = 1.0;
  ...
  while (error > convergence && count < 1000) {
    ...
    const double fleck_c =
        1.0 / (1.0 + constants::a * constants::c * sigma_a * 4.0 * Tc * Tc * Tc * dt / cv);
    ...
    const double dqdT =
        4.0 * sigma_a * constants::a * constants::c * dt * Tstar * Tstar * Tstar + cv;
    Tstar = (Tstar - q / dqdT);
