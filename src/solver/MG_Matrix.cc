 ...
 */
//================================================================================================//
MG_Matrix::MG_Matrix(const Control_Data &control_data, const bool flux_limiter_)
 ...
 */
//================================================================================================//
double MG_Matrix::mass_average(const std::vector<double> &mass,
 ...
 */
//================================================================================================//
void MG_Matrix::initialize_solver_data(const Orthogonal_Mesh &mesh, const Mat_Data &mat_data,
                                       const double dt) {
 ...
 */
//================================================================================================//
void MG_Matrix::build_matrix(const Orthogonal_Mesh &mesh, const double dt) {
  const auto ncells = mesh.number_of_local_cells();
  ...
  for (size_t cell = 0; cell < ncells; cell++) {
    ...
    for (size_t face = 0; face < nfaces; face++) {
      ...
      if (ftype == FACE_TYPE::GHOST_FACE) {
        ...
        const auto rank = gcomm->put_map[cell][face].first;
        const auto put_index = gcomm->put_map[cell][face].second;
        const auto ghost_eden = std::vector<double>(
            solver_data.ghost_cell_eden.begin() + gcomm->ghost_map[next_cell][next_face] * ngroups,
            solver_data.ghost_cell_eden.begin() + gcomm->ghost_map[next_cell][next_face] * ngroups +
                ngroups);

        for (size_t g = 0; g < ngroups; g++) {
          const double deden = std::abs(cell_eden[g] - ghost_eden[g]) / half_width;

          face_D[cell][face][g] =
              flux_limiter ? 1.0 / std::sqrt(9.0 * sigma_tr[g] * sigma_tr[g] +
                                             (deden * deden) / (cell_eden[g] * cell_eden[g]))
                           : 1.0 / (3.0 * sigma_tr[g]);
          // populate the put buffer
        ...
        for (size_t g = 0; g < ngroups; g++) {
          const double sigma_tr = face_sigma_tr[cell][face][g];
          const auto max_eden = std::max(cell_eden[g], next_eden[g]);
          const double deden = std::abs(cell_eden[g] - next_eden[g]) / half_width;
          face_D[cell][face][g] = flux_limiter && max_eden > 0.0
                                      ? 1.0 / std::sqrt(9.0 * sigma_tr * sigma_tr +
                                                        (deden * deden) / (max_eden * max_eden))
                                      : 1.0 / (3.0 * sigma_tr);
          if (!(face_D[cell][face][g] > 0.0)) {
            std::cout << sigma_tr << " " << deden << " " << max_eden << std::endl;
          }
          Check(face_D[cell][face][g] > 0.0);
        }
        ...
        Check(solver_data.off_diagonal_id[cell][face] == ncells);
        for (size_t g = 0; g < ngroups; g++) {
          face_D[cell][face][g] = 1.0 / (3.0 * face_sigma_tr[cell][face][g]);
          Check(face_D[cell][face][g] > 0.0);
        }
      }
    }
  ...
  for (size_t cell = 0; cell < ncells; cell++) {
    ...
    for (size_t g = 0; g < ngroups; g++) {
      solver_data.diagonal[cell][g] = 1.0 + constants::c * dt * sigma_a[cell][g];
      ...
      if (ftype == FACE_TYPE::GHOST_FACE) {
        ...
        if (!reflect_bnd[face]) {
          ...
          for (size_t g = 0; g < ngroups; g++) {
            ...
            solver_data.diagonal[cell][g] += fring;
            solver_data.off_diagonal[cell][face][g] = -fring;
            solver_data.flux_source[cell][face][g] = fring * E0;
          }
        }
      }
    }
 ...
 */
//================================================================================================//
void MG_Matrix::gs_solver(const double eps, const size_t max_iter, const bool print) {
  Require(max_iter > 0);
  ...
  while (max_error > eps && global_count < max_iter) {
    ...
    while (max_error > eps && inner_count < max_iter) {
      ...
      for (size_t i = 0; i < solver_data.diagonal.size(); i++) {
        ...
        if (correction && new_cell_eden > 0.0) {
 ...
 */
//================================================================================================//
void MG_Matrix::calculate_output_data(const Orthogonal_Mesh &mesh, const Mat_Data &mat_data,
