#--------------------------------------------*-cmake-*---------------------------------------------#
# file   odd/autodoc/generate_mainpage_dcc.cmake
# author Mathew Cleveland
# date   October 21st 2021
# brief  Generate mainpage.dcc during 'make autodoc'
# note   Copyright (C) 2018-2021 Triad National Security, LLC.
#        All rights reserved.
# ------------------------------------------------------------------------------------------------ #

# Use: cmake -DINFILE=$infile -DOUTFILE=$outfile -DSUBST_VARIABLE=VALUE ... \ -P
# generate_mainpage_dcc.cmake
#
# Prerequisits: - INFILE must be a valid file (e.g. mainpage.dcc.in)
#
# Post - OUTFILE will be written (or overwritten).
#
# Suggested use is to generate binary directory files based on build-time changes to corresonding
# source tree .in file. For example, add_custom_command( OUTPUT
# "${PROJECT_BINARY_DIR}/autodoc/mainpage.dcc" COMMAND "${CMAKE_COMMAND}"
...
# -DPACKAGE_LINKS=${PACKAGE_LINKS} -P "${PROJECT_SOURCE_DIR}/config/generate_mainpage_dcc.cmake"
# DEPENDS "${PROJECT_SOURCE_DIR}/autodoc/mainpage.dcc.in" )
#
# add_custom_target( mytarget DEPENDS "${PROJECT_BINARY_DIR}/autodoc/mainpage.dcc" COMMENT "Building
# Doxygen mainpage.dcc (HTML)..." )
...
# INPUT is a list of paths message("INPUT = ${INPUT}") string( REGEX REPLACE "[ ]" ";" INPUT
# ${INPUT} ) message("INPUT = ${INPUT}") foreach( item ${INPUT} ) file( TO_NATIVE_PATH "${item}"
# input_path ) set( tmp_input "${tmp_input} ${input_path}" ) message(" ${item} --> ${tmp_input}
# INPUT = ${input_path} ") endforeach() set( INPUT ${tmp_input} ) TAGFILES is a list of paths
if(TAGFILES)
...
if(EXISTS "${IMC_INFO}")
  execute_process(COMMAND "${IMC_INFO}" -a -d OUTPUT_VARIABLE AUTHOR_LIST)
endif()
